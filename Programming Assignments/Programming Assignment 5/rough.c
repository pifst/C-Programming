...
int main(void)
{
    ...
    printf ("The election winner is ID number %d with: %i votes\n", candidateID[winnerIndex], votesReceived[winnerIndex]);

}

/* when calling the function pass the arays to the function using array subscript notation and use array subscript notation in all of your procesing statements in the function. */
void fillArrays(int array1[], int array2[], int count)
{
    int i;
    ...
    for ( i = 0; i < count; ++i ) {
        ...
        do {
            ...
            if ( array2[i] > 0 )
              ...
              printf ("\t*Error: Input %d, is out of range.*\n\tPlease use a number 0 or higher.\n", array2[i]);
        }
        while (1);
    }
}

/* When calling displayArrays pass the arrays as pointers and use "pointer reference notation ex: *(ptr + i)" */
void displayArrays(int *array1, int *array2, int count)
{
    int i;
...
/* For countVotes pass the arrays as pointers to the functions and use "pointer arithmetic" ex: *ptr++ for summing up the votes. */
int countVotes(int *array, int count)
{
    int i = 0;
    int sum = 0;
